...
const api: ElectronAPI = {
  ...
  saveThumbnail(bucket, key, eTag, dataUrl) {
    return ipcRenderer.invoke(CHANNELS.THUMB_SAVE, {
      bucket,
      key,
      eTag,
      dataUrl,
  ...
  uploadObject(prefix, filePath, fileName, id) {
    ...
    return ipcRenderer
      .invoke(CHANNELS.UPLOAD, {
        prefix,
        filePath,
        fileName,
        id: uploadId,
        channel: CHANNELS.UPLOAD_PROGRESS,
      })
      .then((result) => ({ ...(result as object), id: uploadId })) as Promise<{
      key: string;
      id: string;
    }>;
  },
...
// EXPONER INMEDIATAMENTE - ANTES de cualquier evento DOM
contextBridge.exposeInMainWorld("electronAPI", api);
console.log("✅ electronAPI expuesto en main world");

// Verificar que se expuso correctamente
if (typeof window !== "undefined" && (window as any).electronAPI) {
  console.log("✅ Confirmado: electronAPI disponible en window");
} else {
  console.error("❌ ERROR: electronAPI NO se expuso correctamente");
}
